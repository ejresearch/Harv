...
def create_test_data():
    ...
    try:
        ...
        # Create test user with username (check if columns exist first)
        cursor.execute("PRAGMA table_info(users)")
        user_columns = [column[1] for column in cursor.fetchall()]
        
        if 'username' in user_columns and 'created_at' in user_columns:
            cursor.execute("""
            ...
            cursor.execute("""
                ...
                VALUES (1, 'test@example.com', 'hashed_password_123')
            """)
